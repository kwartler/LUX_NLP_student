...
# Or load it from csv, only for educational purposes
...
sentimentTally <- aggregate(count~sentiment + source, nrcSent, sum)
emos <- reshape2::dcast(sentimentTally, sentiment ~source)
emos
...
oneSource <- subset(combinedData, combinedData$source== sourceID[3,1]) #fox
...
ggplot(oneSource, aes(sentiment, as.factor(clusterAssignment), 
  ...
  geom_point() +
  ggtitle(sourceID[3,1], sub = "Emotion by Topic Cluster") + ylab("") +
  theme_tufte()
# End